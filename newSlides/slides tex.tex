...
\frame{
    ...
    \[
    E_R(\beta) = \sum_{i=1}^n{\Big(y_i-\beta_0 - \sum_{j=1}^p {\beta_j x_{ij}}\Big)^2} + \lambda \sum_{j=1}^p {\beta_j^2} = OLS + \lambda \sum_{j=1}^p {\beta_j^2}
    \]
    the least squares component is being minimized such that 
    \[
...
\frame{
    ...
    \[
    E_R(\beta)=\sum_{i=1}^n{\Big(y_i-\beta_0 - \sum_{j=1}^p {\beta_j x_{ij}}\Big)^2} + \lambda \sum_{j=1}^p {|\beta_j|} = OLS + \lambda \sum_{j=1}^p {|\beta_j|}
    \]
    is minimizing the least squares component given the condition that 
    \[
...
\frame{
    \begin{itemize}
        Tuning parameter $t$ or $\lambda(t)$ directly determines the amount of shrinkage applied to the OLS estimates. Consider the OLS estimates as vector  
        \linebreak $\hat{\beta}^o = 
        \begin{bmatrix}
           \hat{\beta}_{1} \\
           \hat{\beta}_{2} \\
           \vdots \\
           \hat{\beta}_{p}
         \end{bmatrix}$ 
         \linebreak Define $t_{o} = \sum_{j=1}^p |\beta_{j}|$. at $t \geq t_o$ we recover the OLS estimates
    \end{itemize}
}
